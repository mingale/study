...
<c:if test="${carts != null}">
	<form action="cartPro.go?cmd=up" name="cartForm" method="post">
		<div class="cart_list">
			<ul>
				...
				<c:forEach var="cart" items="${carts}">
					<li>
						<ul class="clearfix">
							<li class="chk"><input type="checkbox" name="cart_check" value="${cart.no}" class="list"></li>
							<li class="title">
							...
							<li class="item_btn_area">
								<input type="button" value="삭제" onclick="window.location='cartPro.go?cmd=de&no=${cart.no}';">
							</li>
							<li class="total">${cart.price * cart.count}</li>
							<li class="count">
								<input type="hidden" name="no" value="${cart.no}">
								<input class="inputNumber" type="number" name="count" min="1" max="100" value="${cart.count}">
		...
		<div class="btn_area clearfix">
			<input class="smallButton" type="button" value="선택 도서 삭제">
			<input class="smallButton" type="button" value="선택 도서 찜하기">
		</div>
