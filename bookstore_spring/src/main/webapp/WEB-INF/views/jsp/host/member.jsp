...
<div class="wrap clearfix">
	<c:if test="${cnt != 0}">
		<div class="container clearfix">
			<form name="memUpForm">  <!--  action="memberUpdate" -->
				<ul class="content">
					<li>
						...
						<li>
							<ul class="item clearfix">
								...
								<li class="item_btn_area">
									...
									<input class="boardButton" type="button" value="강퇴" onclick="window.location='memberDelete?chk=0&id=${mem.id}';">
								</li>
							</ul>
						</li>
					</c:forEach>
			...
			<div class="paging">
				<c:if test="${startPage > pageNav}">
					<a href="member">시작</a>
					<a href="member?pageNum=${startPage - pageNav}">전</a>
				</c:if>
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<a href="member?pageNum=${i}">${i}</a>
				</c:forEach>
				<c:if test="${endPage < pageCnt}">
					<a href="member?pageNum=${endPage + 1}">후</a>
					<a href="member?pageNum=${pageCnt}">끝</a>
				</c:if>
			</div>
		</div>
	</c:if>
