...
<div class="wrap clearfix">
	...
	<form name="stockForm" method="post">
			...
			<c:forEach var="book" items="${books}">
				<li>
					<ul class="item clearfix">
						...
						<li class="title"><a href="detail?no=${book.no}">${book.title}</a></li>
						<li class="author">${book.author}</li>
						<li class="price">${book.price}</li>
						<li class="count">${book.count}</li>
						<li class="date">${book.bookSub.add_date}</li>
						<li class="item_btn_area">
							<input class="boardButton" type="button" name="btn_up" value="수정" onclick="window.location='stockUpdate?no=${book.no}';">
		...
		<div class="btn_area">
			...
			<input class="smallButton s2" type="button" name="add" value="추가" onmouseover="navOver('s2');" onmouseout="navOut('s2');" onclick="window.location='stockAdd?no=-1';">
		</div>
	
		<div class="paging">
			<c:if test="${startPage > pageNav}">
				<a href="stock">처음</a>
				<a href="stock?pageNum=${startPage - pageNav}">전</a>
			</c:if>
			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				<c:if test="${i == currentPage}">
					<a class="current" href="stock?pageNum=${i}">${i}</a>
				</c:if>
				<c:if test="${i != currentPage}">
					<a href="stock?pageNum=${i}">${i}</a>
				</c:if>
			</c:forEach>
			<c:if test="${pageCount > endPage}">
				<a href="stock?pageNum=${startPage + pageNav}">후</a>
				<a href="stock?pageNum=${pageCount}">끝</a>
			</c:if>
		</div>
	</form>
	</c:if>
	...
	<c:if test="${cnt == 0}">
		<div class="btn_area">
			<input type="button" name="add" value="추가" onclick="window.location='stockAdd?no=-1';">
		</div>
	</c:if>
	</div>
</div>
