...
<c:if test="${carts.size() > 0}">
	<form action="cartPro?cmd=up" name="cartForm" method="post">
		<div class="cart_list">
			<ul>
				<li>
				...
				<c:forEach var="cart" items="${carts}">
					<li>
						<ul class="clearfix">
							...
							<li class="title">
								<a href="detail?no=${cart.no}" class="clearfix">
							...
							<li class="total">${cart.cart_price * cart.cart_count}</li>
							<li class="count">
								<input type="hidden" name="no" value="${cart.no}">
								<input class="inputNumber" id="${cart.no}" type="number" name="count" min="1" max="100" value="${cart.cart_count}" onchange="cartCountChange('${cart.no}');">
							</li>
							<li class="price">${cart.cart_price}</li>
						</ul>
					</li>
				</c:forEach>
			</ul>
