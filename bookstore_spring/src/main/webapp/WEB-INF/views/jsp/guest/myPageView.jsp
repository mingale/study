...
<c:if test="${sessionScope.memId != null}">
	...
	<c:if test="${cnt == 1}">
		...
		<%@ include file="../guest/myPageNav.jsp" %>
			<form class="content" action="myPagePro" name="signUpForm" onsubmit="return signUpCheck();">
				<div class="dv">
				...
				<div class="dv">
					...
					<input class="input" type="text" name="email" value="${m.getEmail()}" onblur="confirmEmail();">
					<input class="input" type="text" name="address" value="${m.getAddress()}">
				</div>
			
				<div class="btn_area clearfix">
					<button class="inputButton" type="submit">수정</button>
					<input class="memout" type="button" value="회원 탈퇴" onclick="window.location='memOutPro'">
				</div>
			</form>
		</div>
		</section>
