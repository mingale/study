...
function idPwdFindView() {
	...
	window.open("signFindView.do", "아이디/비밀번호 찾기", "menubar=no, top=" + winY + ", left=" + winX +", width=500, height=160, resizable=no");
}
...
function confirmId() {
	if(document.signUpForm.id.value) {
		window.location="confirmId.do?id=" + document.signUpForm.id.value;
	}
}
...
function confirmEmail() {
	...
	if(email) {
		if(!emailRegex.test(email)) {
			...
			window.location="confirmEmail.do?email=" + document.signUpForm.email.value;
		}
	}
}
...
function stockDelete(no){
	if(confirm('정말 삭제하시겠습니까?')){
		console.log(no);
		window.location='stockDelete.ho?chk=0&no=' + no;
	}
}
...
function cartCountChange(no) {
	...
	window.location="cartPro.go?cmd=up&no=" + no + "&count=" + count;
}

function cartDelete(no) {
	window.location='cartPro.go?cmd=de&no=' + no;
}
...
function cartCheckDelete() {
	...
	if(nos.length > 0) {
		if(confirm("정말 삭제하시겠습니까?")) {
			var no = nos.join(",");
			window.location="cartPro.go?cmd=de&nos=" + no;
		}
	} else {
		alert("삭제할 도서를 선택하세요.");
	}
...
function myOrderRefund(num) {
	if(confirm(refundConfirm)) {
		window.location="myOrderRefund.go?order_num=" + num;
	}
}
...
function orderDetailView(num) {
	window.open('orderDetail.go?order_num=' + num,'상세페이지','width=500,height=440')
}
...
function hostOrderCheckOk() {
	...
	if(arr.length > 0) {
		if(flg) {
			var order = arr.join(",");
			window.location="orderOk.ho?order_nums=" + order;
			alert("승인");
		} else {
...
function hostOrderCheckDel() {
	...
	if(flg) {
		var order = arr.join(",");
		window.location="orderDelete.ho?order_nums=" + order;
	} else {
		alert("삭제할 주문을 선택하세요");	
	}
}
...
function hostOrderShipping(num) {
	...
	window.open("orderShipping.ho?order_num=" + num, "ShippingStart", "menubar=no, top=" + winY + ", left=" + winX +", width=500, height=190, resizable=no");
}
