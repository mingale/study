			...
			<c:forEach var="order" items="${orders}">
				<li>
					<ul class="item clearfix" data-check="${order.order_num}" data-state="${order.order_state}">
						...
						<li class="title">
							<a href="#" onclick="orderDetailView(${order.order_num})">${order.nos}</a>
						</li>
						<li class="state">${order.getState()}</li>
						<li class="item_btn_area">
							<c:if test="${order.order_state == 0}">
								<input class="btn1 boardButton" type="button" value="승인" onclick="window.location='orderOk.ho?order_num=${order.order_num}';">
							</c:if>
