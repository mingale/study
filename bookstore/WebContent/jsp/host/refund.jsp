	...
	<div class="contain clearfix">
			...
			<c:forEach var="o" items="${refunds}">
				<li>
					<ul class="item clearfix">
						...
						<li class="id">${o.id}</li>
						<li class="title">
							<a href="#" onclick="orderDetailView('${o.order_num}')">${o.nos}</a>
						</li>
						<li class="state">${o.getState()}</li>
						<li class="item_btn_area">
							<c:if test="${o.order_state == 8}">
								<input class="boardButton" type="button" value="승인" onclick="window.location='refundOk.ho?order_num=${o.order_num}'">
								<input class="boardButton" type="button" value="거부" onclick="window.location='refundNo.ho?order_num=${o.order_num}'">
							</c:if>
						</li>
					</ul>
				</li>
		...
		<div class="paging">
			<c:if test="${startPage > pageNav}">
				<a href="refund.ho">시작</a>
				<a href="refund.ho?pageNum=${startPage - pageNav}">전</a>
			</c:if>
			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				<a href="refund.ho?pageNum=${i}">${i}</a>
			</c:forEach>
			<c:if test="${pageCnt > endPage}">
				<a href="refund.ho?pageNum=${endPage + pageNav}">후</a>
				<a href="refund.ho?pageNum=${pageCnt}">끝</a>
			</c:if>
		</div>
